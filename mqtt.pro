...
UBUNTU_TRANSLATION_DOMAIN="mqtt.liu-xiao-guo"
...
aptest.commands = QML2_IMPORT_PATH=$$OUT_PWD/backend bash $$PWD/mqtt/tests/autopilot/run
aptest.depends  = sub-mqtt sub-backend-App

unittest.target   = check
unittest.commands = /usr/bin/qmltestrunner -input $$PWD/backend/tests/unit -import $$OUT_PWD/backend
unittest.commands += && /usr/bin/qmltestrunner -input $$PWD/mqtt/tests/unit -import $$OUT_PWD/backend
unittest.depends  = sub-mqtt sub-backend-App
